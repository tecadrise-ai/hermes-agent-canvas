(function () {
  ...
  function makeAgentFromProfile(info, previous) {
    ...
    return {
      ...
      taskEdges: [],
      chat:
        prev && Array.isArray(prev.chat) && prev.chat.length
          ? prev.chat
          : [
              {
                role: "meta",
  ...
  function renderSideMenu() {
    ...
    for (const block of BLOCK_CATALOG) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "side-item" + (onCanvas.has(block.id) ? " is-on-canvas" : "");
      btn.setAttribute("aria-current", block.id === current ? "true" : "false");
      btn.title = onCanvas.has(block.id)
        ? block.label + " (on canvas)"
        : block.label + " — double-click to add";
  ...
  function handleChatFrame(frame) {
    ...
    if (ev.type === "message.complete") {
      ...
      if (st.streamingIdx != null && agent.chat[st.streamingIdx]) {
        ...
        agent.chat.push({ role: "assistant", text: finalText });
      }
  ...
  async function ensureChatSession(agent) {
    const st = chatGw.sessions[agent.id] || {
      sessionId: null,
      streamingIdx: null,
      thinkingMsg: null,
  ...
  async function sendChat() {
    ...
    const agent = activeAgent();
    agent.chat.push({ role: "user", text: text });
    elInput.value = "";
    renderChat();
    ...
    try {
      ...
      const sessionId = await ensureChatSession(agent);
      const st = chatGw.sessions[agent.id];
      st.thinkingMsg = thinking;
      thinking.text = "Hermes is generating…";
      renderChat();
      // Do not pre-create empty assistant bubble; first delta/start will.
      st.streamingIdx = null;
      await chatRpc("prompt.submit", { session_id: sessionId, text: text }, 1800000);
      if (st.thinkingMsg && agent.chat.includes(st.thinkingMsg)) {
        // still waiting on stream events; leave the status until complete
      }
    } catch (err) {
  ...
  function setEditorChrome(kind, mode) {
    ...
    const showText = !showForm && !showSecrets && !showSkills;
  ...
  async function boot() {
    ...
    // Auto-attach only when a remote URL + token were saved (Desktop remote path).
    let connected = false;
    try {
